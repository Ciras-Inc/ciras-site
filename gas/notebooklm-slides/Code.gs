 ...
 * @param {Object} config - { files, title, shareMode, emails, permission }
 * @return {Object} 作成結果 { id, url, slideCount, title }
 */
function createPresentation(config) {
  ...
  for (var i = 0; i < config.files.length; i++) {
    ...
    if (file.mimeType === 'application/pdf') {
      var pdfImages = extractImagesFromPdf_(blob);
      if (pdfImages.length > 0) {
        imageBlobs = imageBlobs.concat(pdfImages);
      } else {
        // PDF から画像抽出できなかった場合、1ページ目のサムネイルを取得
        var tempFile = DriveApp.createFile(blob);
        try {
          var thumb = tempFile.getThumbnail();
          if (thumb) imageBlobs.push(thumb);
        } finally {
  ...
  if (imageBlobs.length === 0) {
    throw new Error(
      '画像を取得できませんでした。\n' +
      'PDF の場合はページごとに画像（PNG/JPG）として書き出してからアップロードしてください。'
    );
  }
  ...
  for (var i = 0; i < imageBlobs.length; i++) {
    ...
    if (i === 0) {
      slide = firstSlide;
      // デフォルトのプレースホルダーを削除
      var defaultElements = slide.getPageElements();
      for (var d = defaultElements.length - 1; d >= 0; d--) {
        defaultElements[d].remove();
      }
    } else {
      slide = presentation.appendSlide(SlidesApp.PredefinedLayout.BLANK);
    }

    var image = slide.insertImage(imageBlobs[i]);
    image.setLeft(0);
    image.setTop(0);
...
/**
 * PDF を Google Docs に変換し、埋め込み画像を抽出する。
 * NotebookLM の PDF は各ページが画像として変換されることが多い。
 */
function extractImagesFromPdf_(pdfBlob) {
  var images = [];
  ...
  try {
    ...
    Logger.log('PDF → Docs 変換エラー: ' + e.message);
    return images;
  }
...
function collectImages_(element, images) {
  if (element.getType() === DocumentApp.ElementType.INLINE_IMAGE) {
    images.push(element.asInlineImage().getBlob());
    return;
  }
  if (typeof element.getNumChildren === 'function') {
    for (var i = 0; i < element.getNumChildren(); i++) {
      collectImages_(element.getChild(i), images);
    }
  }
}
...
/**
 * 指定スライドの画像を差し替える。
 * @param {Object} imageData - { data, mimeType, fileName } (Base64) or { url }
 */
